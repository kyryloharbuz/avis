...
class FlightSupervisor:
    ...
    def __init__(
        ...
        max_forward=55,            # МЕЖА швидкості руху ВПЕРЕД (ризикова вісь).
                                   #   30 → 40 → 55: дрон наздоганяв мляво.
        max_backward=25,           # МЕЖА швидкості руху НАЗАД, нижча за вперед.
                                   #   Було 15 — але це рівно на межі чутливості
                                   #   Tello, і відступ фактично не працював.
                                   #   Різкість, на яку ти скаржився, лікується не
                                   #   цим, а ЛІНІЙНИМ керуванням у метрах: тепер
                                   #   команда пропорційна, а не завжди насичена.
        min_speed=12,              # поріг чутливості ПОВОРОТУ й вертикалі
        min_forward=20,            # поріг чутливості осі ВПЕРЕД/НАЗАД — вищий.
                                   #   Заміряно: команди 10-12 дрон фізично не
                                   #   відпрацьовував, вісь була мертва.
    ...
    def update(self, command, has_target, dt, blind=False):
        """command — те, що порахував PID (може бути None);
        has_target — чи є зараз надійна ціль (або її прогноз);
        blind — кадр побитий втратою пакетів, тобто інформації про ціль НЕМАЄ.
        ...
        #
        # АЛЕ на побитому кадрі відлік СТОЇТЬ. Тут вирішується безпека — після
        # 0.5 с без цілі дрон зависає, після 8 с сідає, — і ці рішення мають
        # спиратись на факт "цілі немає", а не на "відео розсипалось". Заміряно:
        # у довгих втратах цілі 42% кадрів побиті, тобто без цієї умови дрон
        # регулярно зупинявся й сідав через перешкоди у Wi-Fi, а не через ціль.
        # Рух при цьому не змінюється: нижче ми однаково не шлемо PID наосліп.
        if not blind:
    ...
    def _ramp(self, target: Command, dt) -> Command:
        ...
        # Плавність потрібна ПОВОРОТУ — саме він смикався. Вісь вперед/назад
        # НЕ обмежуємо: там команда й так рідка, а обмеження наростання разом
        # із частими нулями не давало їй дорости до дієвої величини — вісь
        # виходила мертвою. Вертикаль теж лишаємо плавною.
    ...
    def _shape(self, v, ceiling, floor=None):
        ...
        ЧОМУ ТУТ НЕМАЄ "порогу зобов'язання". Я спробував обнуляти команди,
        слабші за пів-порогу, — і це зламало підхід: дрон зупинявся вже на
        1.2 м, бо регулятор просив 7.5, а поріг 20. Гірше — так само глухнув
        і ПРИМУСОВИЙ ВІДСТУП запобіжника.
        Правильне рішення інше: раз ми ВИЙШЛИ із зони спокою, значить рух
        справді потрібен, і підтягнути до порогу — чесно. А щоб дрон не
        проскакував ціль, ЗОНА СПОКОЮ зроблена ширшою за гальмівний шлях
        (0.20 м проти ~0.11 м) — тоді "все або нічого" сходиться в межах зони."""
        if v == 0:
            return 0.0
        floor = self._min_speed if floor is None else floor
        # Поріг чутливості не може бути ВИЩИМ за межу — інакше він її перебивав
        # би, і команда виходила б більшою за дозволену.
        floor = min(floor, ceiling)
        v = max(-ceiling, min(v, ceiling))        # обрізання зверху
        if abs(v) < floor:                        # підтягування знизу
            v = floor if v > 0 else -floor
        return v
